...
export function registerListCommands({
  ...
  const commands = {
    ...
    async add() {
      ...
      const urlText = localize.getLocalize('text.jsonUrl')
      ...
      config.extConfig.jsonUrl.forEach((v) => {
        if (v.url === url) {
      ...
      const jsonUrl = Object.assign([], config.extConfig.jsonUrl || [])
      jsonUrl.push(deleteEmptyProperty({ title, url, savePath }))
      config.setCodeConfig({ jsonUrl })
