...
export class ViewList extends BaseTreeProvider<ListItem> {
  ...
  async getChildren(element?: ListItem) {
    if (!element) {
      const { jsonUrl = [] } = config.extConfig
      return jsonUrl.map((item) => this.renderRootItem(item))
    }
  ...
  public getSearchList(): Promise<ListPickerItem[]> {
    ...
    return new Promise(async (resolve) => {
      ...
      this.swaggerJsonMap.forEach((list, key) => {
        const conf = jsonUrl.find((x) => x.url === key)
        if (!conf) return log.error(`jsonUrl config not found (${key})`)
  ...
  getParent(item: ListItem): ProviderResult<ListItem> {
    ...
    switch (type) {
      ...
      case 'group':
        const rootNode = config.extConfig.jsonUrl.find((x) => x.url === parentKey)
        if (rootNode) {
